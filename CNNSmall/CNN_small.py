...
model_original.eval()
model_original.load_state_dict(torch.load("CNN_small"))
model_original.eval()
...
def create_input_json(buffer, computed, samples, dir):
    ...
    for i in range(1,samples+1):
        ...
        with open(dir + "inputs/input" + str(i) + ".json", "w") as outfile:
...
d = {}
d["numInChannels"] = lin.out_features
d["numOutChannels"] = lin.out_features
d["channelHeight"] = height
d["channelWidth"] = width
...
if True:
    ...
    with open("Configs/softmax.json", "w") as outfile:
        outfile.write(lin_json)
    create_input_json(lin_output, softmax_output, batch_size, "Tests/softmax/")